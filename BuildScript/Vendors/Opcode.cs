...
{
	public class Opcode : ThirdParty
	{
		public Opcode( ProjectFile project, PlatformType platform, Configuration configuration )
			: base( project, platform, configuration )
		{
			...
			project.IncludePath( "%(VendorsDir)OPCODE/Src" );
			project.Library(string.Format("%(VendorsDir)OPCODE/Build/{0}/lib/OPCODE_{1}_{2}", vsName, platformName, configuration.target.ToString()));
		}
	}
}